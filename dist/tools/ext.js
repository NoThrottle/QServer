"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.generatePepper = exports.Get512Hash = exports.generateRandomHexString = exports.RSAEncrypt = exports.Error = exports.RSADecrypt = exports.salt = exports.publicKey = exports.signature = void 0;
const crypto_1 = __importDefault(require("crypto"));
exports.signature = "";
const privateKey = `
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;
exports.publicKey = `
-----BEGIN PUBLIC KEY-----

EQIDAQAB
-----END PUBLIC KEY-----`;
exports.salt = `8270ebcf-22e2-491b-84b8-d96f0f2e5edb`;
function RSADecrypt(value) {
    const decipher = crypto_1.default.privateDecrypt(privateKey, Buffer.from(value, 'base64'));
    return decipher.toString('utf-8');
}
exports.RSADecrypt = RSADecrypt;
;
function Error(value) {
    return JSON.stringify({ "Error": value });
}
exports.Error = Error;
function RSAEncrypt(value) {
    const cipher = crypto_1.default.publicEncrypt(exports.publicKey, Buffer.from(value, 'utf-8'));
    return cipher.toString('base64');
}
exports.RSAEncrypt = RSAEncrypt;
;
function generateRandomHexString(length) {
    let buffer = crypto_1.default.randomBytes(length / 2);
    // Convert the buffer to a hex string
    const hexString = Array.from(buffer)
        .map((byte) => byte.toString(16).padStart(2, '0'))
        .join('');
    return hexString;
}
exports.generateRandomHexString = generateRandomHexString;
function Get512Hash(value) {
    let sha512Hash = crypto_1.default.createHash('sha512');
    sha512Hash.update(value, 'utf-8');
    return sha512Hash.digest('hex');
}
exports.Get512Hash = Get512Hash;
;
function generatePepper() {
    const num = Math.floor(Math.random() * (268435455 - 16777216 + 1) + 16777216);
    return num.toString(16).toLowerCase();
}
exports.generatePepper = generatePepper;
String.prototype.isAlphanumericUnderscore = function () {
    return alphanumeric(String(this));
};
function alphanumeric(input) {
    for (let i = 0; i < input.length; i++) {
        const char1 = input.charAt(i);
        const cc = char1.charCodeAt(0);
        if ((cc >= 48 && cc <= 57) || // 0-9
            (cc >= 65 && cc <= 90) || // A-Z
            (cc >= 97 && cc <= 122) || // a-z
            cc === 95 // underscore (_)
        ) {
        }
        else {
            return false;
        }
    }
    return true;
}
//Below is unused
function generateKeyPair() {
    crypto_1.default.generateKeyPair('rsa', keyOptions, (err, publicKey, privateKey) => {
        if (err) {
            console.error('Error generating key pair:', err);
        }
        else {
            console.log('Public Key:', publicKey);
            console.log('Private Key:', privateKey);
        }
    });
}
;
const keyOptions = {
    modulusLength: 2048,
    publicKeyEncoding: {
        type: 'spki',
        format: 'pem'
    },
    privateKeyEncoding: {
        type: 'pkcs8',
        format: 'pem'
    }
};
